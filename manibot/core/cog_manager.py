...
from manibot import command, group

class CogManager:
    """Commands to add, remove and change cogs for Bot."""
    ...
    async def load_extension(self, ctx, name, extension):
        ...
        if ext not in self.available_exts and ext_loc == "manibot.cogs":
    ...
    async def _list(self, ctx):
        ...
        status_list = []
        for ext in all_exts:
            is_loaded = f"manibot.cogs.{ext}" in ctx.bot.extensions
    ...
    async def unload(self, ctx, extension):
        """Unload an extension."""
        ext_name = f"manibot.cogs.{extension}"
    ...
    async def load(self, ctx, *extensions):
        ...
        for ext in extensions:
            ...
            await self.load_extension(ctx, name, f"manibot.cogs.{ext}")

    @load.command(name="core")
    async def _core(self, ctx):
        """Reload Core Commands."""
        await self.load_extension(ctx, 'Core Commands', 'manibot.core.commands')

    @load.command(name="cm")
    async def _cm(self, ctx):
        """Reload Cog Manager."""
        await self.load_extension(ctx, 'Cog Manager', 'manibot.core.cog_manager')
