...
from manibot.utils import Map

class NewCog:
    def __init__(self, name, callback, extension, **kwargs):
        if not isinstance(name, str):
            raise TypeError('Name of a cog must be a string.')
        self.name = name
        ...
        self.check = getattr(callback, '__local_check', None)
        self._before_invoke = getattr(callback, '__local_check', None)
        self._before_invoke = getattr(callback, '__before_invoke', None)
        self._after_invoke = getattr(callback, '__after_invoke', None)
