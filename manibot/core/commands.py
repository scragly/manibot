...
from manibot import command, group, checks
from manibot.core.data_manager import errors
from manibot.utils import snowflake
from manibot.utils.pagination import Pagination
from manibot.utils.formatters import url_color, make_embed, convert_to_bool
...
class Core:
    ...
    async def _about(self, ctx):
        ...
        about = [
            "Hatigarm's Community Bot!",
            f"**[Docs & Source]({bot_repo})**\n",
            # f"**{invite_str}**\n",
            f"Stats: `{ctx.prefix}stats`",
            f"Help: `{ctx.prefix}help`\n",
            ("I'm currently in Beta testing!\n"
             f"Subscribe to updates with `{ctx.prefix}sub`\n"),
            f"**Owner:** {owner}",
            f"**Uptime:** {uptime_str}",
    ...
    async def _stats(self, ctx):
        ...
        dpy_version = bot.dpy_version.split('+')
        instance_msg = (
            f"**Uptime:** {uptime_str}\n"
            f"**Version:** {bot.version}\n"
            f"**Lib Ver:** {dpy_version[0]}\n"
            f"**Lib GitHash:** {dpy_version[1]}\n"
        ...
        session_msg = (
            ...
            f"**Reconnects:** {bot.resumed_count}")
        process_msg = (
            f"**PID:** {ppid}\n"
            f"**RAM:** {p_mem_str}\n"
            f"**SysRAM:** {mem_p}\n"
    ...
    async def _ping(self, ctx):
        ...
        for shard, latency in ctx.bot.latencies:
            shard_str = ''
            here = ''
            if len(ctx.bot.shards) > 1:
                shard_str = f"**Shard {shard}:** "
                here = '  \U0001f448' if shard == ctx.guild.shard_id else ''
            data.append(f"{shard_str}{(latency* 1000):.2f} ms{here}")
        await ctx.embed('Latency', '\n'.join(data), msg_type='info')
    ...
    async def reload_cm(self, ctx):
        ...
        bot = ctx.bot
        try:
            bot.unload_extension('manibot.core.cog_manager')
